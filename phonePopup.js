(function() {
        ...
        actionButtons.innerHTML = `
            ...
            <button class="action-button" data-location="SouthCreek">
                <span class="status-indicator"></span>
                SouthCreek: Open
            </button>
        `;
        ...
        // Replace the click event listener with this:
        let isOpen = false;
        ...
        function shouldFetchNewData() {
            if (!hoursCache.lastFetch) return true;

            const now = new Date();
            const lastFetch = new Date(hoursCache.lastFetch);
        ...
        // Function to fetch location data with caching
        async function fetchHoursData() {
            if (!shouldFetchNewData() && hoursCache.data) {
            ...
            try {
                ...
                */   
                ...
                // Update cache
                hoursCache.data = data;
                hoursCache.lastFetch = new Date().toISOString();
    ...
    // Function to check location status using cached data
    async function checkLocationStatus(location) {
        const data = await fetchHoursData();
        if (!data) return null;
    ...
    async function updateStatusIndicator(button) {
        ...
        if (!locationData) {
            ...
            button.innerHTML = `<span class="status-indicator status-warning"></span>${location}: Call for Hours`;
        ...
        // Destructure time values from locationData
        const { 'opening-soon': openingSoon, 'open': openTime, 
                'closing-soon': closingSoon, 'close': closeTime } = locationData;

        // Check if all values are null (closed for the day)
        if (openingSoon === null && openTime === null && 
            closingSoon === null && closeTime === null) {
            indicator.className = 'status-indicator status-closed';
            button.innerHTML = `<span class="status-indicator status-closed"></span>${location}: Closed`;
        ...
        // Update the button's HTML and classes
        indicator.className = `status-indicator ${statusClass}`;
        button.innerHTML = `<span class="status-indicator ${statusClass}"></span>${location}: ${status}`;
